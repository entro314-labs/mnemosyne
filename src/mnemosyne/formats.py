...
def render_jsonl(
    ...
    *,
    ...
    - ``role`` — ``"user"`` / ``"assistant"``, or ``"attachment"`` for a
      standalone attachment record (only emitted with ``include_attachments``).
...
# The renderer grows its fences past three backticks whenever the content itself
# contains a fence (see `render._fence`), so these must match three-or-more —
# a fixed ``` would leave the generated delimiter in "plain" output.
_FENCE_OPEN_RE = re.compile(r"`{3,}[\w-]*\n")
_FENCE_CLOSE_RE = re.compile(r"\n`{3,}")
# `<details><summary>…</summary>` wrappers are generated by the markdown renderer
# for thinking blocks and collapsed tool output; they are markup we emitted, so
# plain output must drop them (the summary text is kept as a label).
_DETAILS_OPEN_RE = re.compile(r"<details>\s*<summary>(.*?)</summary>", re.DOTALL)
_DETAILS_CLOSE_RE = re.compile(r"\s*</details>")
...
def _strip_markdown(body: str) -> str:
    ...
    Removes only decoration that *mnemosyne* generated. Original user/assistant
    text may itself contain markdown; that is content, and is left intact.
    """
    body = _TOOL_LABEL_RE.sub(r"[\1]", body)
    body = _DETAILS_OPEN_RE.sub(r"[\1]", body)
    body = _DETAILS_CLOSE_RE.sub("", body)
